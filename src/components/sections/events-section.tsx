...
export default function EventsSection() {
  ...
  return (
    <section id="events" className="min-h-[100dvh] flex items-center py-8 md:py-32 bg-gray-50 dark:bg-zinc-950 relative overflow-hidden transition-colors duration-500">
      ...
      <Container>
        <ScrollReveal>
          <div className="mb-6 md:mb-16 text-center relative z-10">
          ...
          {upcomingEvent ? (
            <div className="max-w-6xl mx-auto relative group perspective-1000">
              ...
              >
                ...
                <div className="relative grid md:grid-cols-12 gap-4 md:gap-8 p-4 md:p-10 z-20">

                  {/* Left: Full Poster Visibility (5 Cols) */}
                  ...
                  <div className="md:col-span-7 flex flex-col justify-center space-y-4 md:space-y-8">
                    <div>
                      {/* Big Date Typography */}
                      <div className="flex items-baseline space-x-2 md:space-x-3 mb-1 md:mb-2 select-none">
                        <span className="text-5xl md:text-9xl font-bold tracking-tighter leading-none text-transparent bg-clip-text bg-gradient-to-b from-foreground to-foreground/10 dark:from-white dark:to-white/10 opacity-80">
                          {upcomingEvent.date.split(' ')[0].replace(/\D/g, '')}
                        </span>
                        <div className="flex flex-col justify-end h-full pb-1 md:pb-4">
                          ...
                          <span className="text-xs md:text-base text-muted-foreground font-medium">
                            {upcomingEvent.date.split(' ').slice(2).join(' ')}
                          </span>
                        </div>
                      </div>
                    ...
                    <div className="space-y-2 md:space-y-4">
                      <div className="flex items-center p-3 md:p-4 rounded-xl bg-white/50 dark:bg-white/5 border border-white/20 dark:border-white/10 backdrop-blur-sm transition-colors hover:bg-white/80 dark:hover:bg-white/10">
                        <div className="p-1.5 md:p-2 rounded-lg bg-primary/10 mr-3 md:mr-4">
                          <MapPin className="h-4 w-4 md:h-6 md:w-6 text-primary" />
                        </div>
                        <span className="text-sm md:text-lg font-medium text-foreground">{upcomingEvent.location}</span>
                      </div>
                      ...
                      <div className="flex items-center p-3 md:p-4 rounded-xl bg-white/50 dark:bg-white/5 border border-white/20 dark:border-white/10 backdrop-blur-sm transition-colors hover:bg-white/80 dark:hover:bg-white/10">
                        ...
                        <span className="text-sm md:text-lg font-medium text-foreground">{upcomingEvent.date}</span>
                      </div>
                    </div>
